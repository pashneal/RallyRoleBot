...
def add_role_coin_mapping(guild_id, coin, required_balance, role):
    global db
    table = db["mappings"]
    table.insert({"guildId" : guild_id, "coinKind": coin, "requiredBalance": required_balance, "role": role})

def get_role_mapping(guild_id, coin=None, required_balance=None, role=None):
    global db
    table = db["mappings"]
    ...
    if required_balance is not None:
        filtered_mappings = [m for m in  filtered_mappings if m["requiredBalance"] == required_balance]
    if role is not None:
